...
class ConanWrapper:
    """
    ConanWrapper provides a Python interface to the shell commands
    required to download Conan packages.
    There is a Python API, but it is not documented, and according
    to: https://github.com/conan-io/conan/issues/6315
    ``The python api is not documented nor stable. It might change
    at any time and break your scripts.``
    This class can be updated to use the Python API if/when that
    makes sense.

    :param folder: destination path for downloaded (deployed) conan
                   packages.
    :type folder: string
    """
    ...
    def download_package(
        ...
        """
        Run conan install to download the conan package and deploy it to
        the folder specified at construction.
        ...
        :type channel: string
        :param profile: name of text file containing the conan profile
                        used to deploy the download package
        :type profile: string
