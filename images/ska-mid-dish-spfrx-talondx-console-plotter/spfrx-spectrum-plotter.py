...
class SpectrumPlotter:
    ...
    def update(self, i):
        ...
        if (self._test_mode):
            ...
            self._pcap.command_read_write(
                "spectrometer_retrieve_result", None
                )
            self._raw = self._pcap.read_attribute(
                "spectrometer_spectrum_result"
                )
            attH = self._ctrl.read_attribute(
                "attenuationPolH"
                )
            attV = self._ctrl.read_attribute(
                "attenuationPolV"
                )
            if attH is None:
                attH = -1
    ...
    def parseData(self):
        ...
        self._data_xx = np.array(
            [self._raw[2:1027], self._raw[4102:5127]]
            )
        self._data_xx = np.where(
            self._data_xx != 0, self._data_xx, 1
            )
        self._data_xx = 10 * np.log10(
            self._data_xx
            )

        self._data_yy = np.array(
            [self._raw[1027:2052], self._raw[5127:6152]]
            )
        self._data_yy = np.where(
            self._data_yy != 0, self._data_yy, 1
            )
        self._data_yy = 10 * np.log10(
            self._data_yy
            )
        ...
        self._data_xy_re = np.where(
            self._data_xy_re != 0, self._data_xy_re, 1
            )

        self._data_xy_im = np.array(
            [self._raw[3077:4102], self._raw[7177:8202]]
            )
        self._data_xy_im = np.where(
            self._data_xy_im != 0, self._data_xy_im, 1
            )
    ...
    def updatePlot(self, timestamp, attH=-1, attV=-1):
        ...
        plt.setp(
            ...
            xticklabels=(np.linspace(range_low, int(range_high), 7, dtype=int))
            )
        if attH >= 0:
            self._ax1.annotate(
                f"Atten: {attH} dB",
        ...
        if mag:
            ...
            data_xy_mag_ndoff = 5.0 * np.log(
                np.sqrt(np.square(self._data_xy_re[1]) +
                        np.square(self._data_xy_im[1]))
                )
            data_xy_mag_ndon = 5.0 * np.log(
                np.sqrt(np.square(self._data_xy_re[0]) +
        ...
        if mag:
            ...
            self._ax3.plot(
                self._data_xy_re[1], 'r',
                self._data_xy_re[0], 'b')
# , data_xy_re[1], 'g', data_xy_im[1], 'y')
        ...
        if mag:
            ...
            self._ax4.plot(
                ...
                'r',
                self._data_xy_im[0],
                'b')  # , data_xy_re[1], 'g', data_xy_im[1], 'y')
        ...
        if mag:
            ...
            self._ax4.plot(
                [0, 1025], piover2, color='black', linestyle='--'
                )
            self._ax4.plot(
                [0, 1025], [0, 0], color='black', linestyle='--'
                )
            self._ax4.plot(
